...
class HostAnalyzer(object):
    MATCH_WITH_SOURCE_COMMON_NAME = True
    ...
    SYNONYM_HOST_NAME = 'HOST_NAME'
    SYNONYN_SYNONYM = 'SYNONYM'
    ...
    @staticmethod
    def read(source_file_name, target_file_name, synonym_file_name):
        match_outfile = open("venv/matches.txt", "w")
        no_match_outfile = open("venv/no_matches.txt", "w")

        match_outfile.write(
            'HOST_NAME\tHOST_COMMON_NAME\tDICT_SCI_NAME\tDICT_COMMON_NAME\tOrder\tFamily\tHost group\tHost subgroup\t'
            + 'Host sub-subgroup\tHost sub-sub-sub-group' + '\n')
        ...
        synonym_df = pd.read_csv(synonym_file_name, dtype={
            HostAnalyzer.SYNONYM_HOST_NAME: str,
            HostAnalyzer.SYNONYN_SYNONYM: str
        })
        ...
        for index, row in source_df.iterrows():
            ...
            found_syn_df = synonym_df.loc[
                synonym_df[HostAnalyzer.SYNONYM_HOST_NAME].str.lower() == source_sci_name.lower()]
            synonym = None
            if not found_syn_df.empty:
                synonym = str(found_syn_df.iloc[0][HostAnalyzer.SYNONYN_SYNONYM])
            ...
            if synonym and not match:
                sci_to_synonym_df = target_df.loc[
                    target_df[HostAnalyzer.TARGET_SCI_NAME].str.lower() == synonym.lower()]
                if not sci_to_synonym_df.empty:
                    match = True
                    match_syn_to_sci += 1
                    if len(sci_to_synonym_df.index) > 1:
                        multiple_match_counter += 1
                    else:
                        HostAnalyzer.write_match(match_outfile, source_sci_name, source_common_name, 'sci_to_SYN',
                                                 sci_to_synonym_df)
        ...
        print('match syn to sci      : ' + str(match_syn_to_sci))
...
if __name__ == "__main__":
    HostAnalyzer.read('C:/Users/czmasek/Dropbox/WORK/JCVI/HOSTS/BVBRC_host_lookup.csv',
                      ...
                      'C:/Users/czmasek/Dropbox/WORK/JCVI/HOSTS/AvianHost_SynonymDictionary.csv')
